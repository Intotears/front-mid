...
const viewRecipe = {
  ...
  actions: {
    ...
    async loadUserRatingUpdate({ commit,getters }, id) {
      const userID = getters.findUserID;
      console.log("loadUserRatingUpdate ", userID);
      await axios
        .get(
          `${process.env.VUE_APP_BACKEND}/api/find/findRatingByUserRecipeID/${id}/${userID}`
        )
        .then((response) => {
